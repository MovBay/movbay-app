    ...
    <View style={styles.modalOverlay}>
      <View style={styles.modalContainer}>
        ...
        <View style={styles.textContainer}>
          <Text style={styles.modalTitle}>KYC Updated Successfully!</Text>
...
const KycUpdate = () => {
  ...
  const { riderKYC, isLoading: isLoadingKYC, refetch } = useRiderKYC()
  ...
  const onSubmit = (data: any) => {
    ...
    // Create rider object with text fields
    const riderData = {
    ...
    // Add rider data as JSON string
    formData.append("rider", JSON.stringify(riderData))
    ...
    console.log("- rider:", JSON.stringify(riderData))
    console.log("- nin:", ninImage ? "Image selected" : "No image")
    console.log("- proof_of_address:", proofOfAddressImage ? "Image selected" : "No image")
    console.log("- drivers_licence:", driversLicenceImage ? "Image selected" : "No image")

    // Submit the form
    updateKYC(formData, {
      onSuccess: (response) => {
        setShowSuccessModal(true)
        console.log("KYC Update Success:", response)
      },
      onError: (error: any) => {
        console.log("KYC Update Error:", error?.response?.data)
        ...
        } else if (error?.response?.data?.rider) {
          toast.show("Invalid rider information", { type: "danger" })
        } else {
    ...
    <SafeAreaView className="flex-1 bg-white">
      ...
      <View className="flex-1">
          ...
          <View className="">
            ...
            <View className="pt-3">
              ...
              <Text className="text-base" style={{ fontFamily: "HankenGrotesk_400Regular" }}>
                Upload the document's below for KYC verification
              </Text>
            </View>
              ...
              <View className="mb-6">
                ...
                <View className="flex-row gap-3 items-center mt-2">
                  ...
                  {driversLicenceImage ? (
                    <View className="w-20 h-14 rounded-lg overflow-hidden">
                      <Image
                        source={{ uri: driversLicenceImage }}
                        style={{ width: "100%", height: "100%", borderRadius: 10, objectFit: "cover" }}
                      />
                    </View>
                  ) : (
        ...
        <View className="px-7 pb-4 pt-2 bg-white border-t border-gray-100">
          <SolidMainButton onPress={handleSubmit(onSubmit)} text={"Submit"} />
        </View>

        {/* Custom Modal */}
        <CustomSuccessModal visible={showSuccessModal} onClose={closeModal} />
      </View>
    </SafeAreaView>
  )
}
