  ...
  ({
    ...
    const handleAddToCart = async () => {
      try {
        ...
        Toast.show(`${title} has been added to your cart`, { type: "success" })
      } catch (error) {
    ...
    return (
      <View className="w-[50%] mb-4">
        <Pressable onPress={() => handleViewProduct(id)}>
          <Animated.View className="w-full relative" entering={FadeInDown.duration(400).delay(200).springify()}>
            <View className="w-full h-48 bg-slate-200 rounded-3xl relative">
              ...
              {/* Cart Button */}
              <Pressable
                className="absolute bottom-2 right-2 p-3 flex justify-center items-center rounded-full bg-[#FEEEE6]"
            ...
            <View>
              <View>
                ...
                <View className="flex-row items-center gap-3">
                  ...
                  <Text
                    className="text-sm pt-2 italic line-through text-neutral-500"
